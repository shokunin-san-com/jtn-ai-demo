...
#### 印刷レイアウト検証
- [ ] 出力Excelを A4縦で印刷プレビューして、1ページに収まる（はみ出さない）
- [ ] ページ設定が「余白: 標準」「スケーリング: 100%」に設定されている
- [ ] ヘッダー・フッターが設定されていない（または必要に応じて設定）
- [ ] 印刷時の改ページが適切（不要な改ページがない）
- [ ] セルの文字が切れていない（スケーリングで無理やり縮小していない）
- [ ] テンプレート本来の装飾（色・背景）が保持されている
